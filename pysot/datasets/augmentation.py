...
class Augmentation:
    ...
    def create_attention_mask(self, atts, ratios, detections):  # ratios = 255/25=0.2
        ...
        y_int = (y / ratios).astype(np.int32)  # in:[255,255]???bbox???[25, 25]???attention map??????????????????
        for att in atts:
            att[0, y_int, x_int] = 1
        x_float = x / ratios
        y_float = y / ratios
    ...
    def __call__(self, image, bbox, size, data, gray=False):
        ...
        #cv2.imwrite('127_255.jpg', image)  # image???[127,127,3] ???[255,255,3]
        crop_bbox = center2corner(Center(shape[0] // 2, shape[1] // 2,
                                         size - 1, size - 1))
        # color augmentation
        ...
        if data == 'template':
            ...
            #print(cfg.corners.crop_size)
            image_l[int(bbox[1]):(int(bbox[3])), int(bbox[0]):(int(bbox[0]+crop_size_l)), :] =\
                image[int(bbox[1]):(int(bbox[3])), int(bbox[0]):(int(bbox[0]+crop_size_l)), :]
            ...
            image_t[int(bbox[1]):(int(bbox[1]+crop_size_t)), int(bbox[0]):(int(bbox[2])), :] = \
                image[int(bbox[1]):(int(bbox[1]+crop_size_t)), int(bbox[0]):(int(bbox[2])), :]
            #cv2.imwrite('crop_t.jpg', image_t)  # image:[255,255,3]
            
            crop_size_b = cfg.corners.crop_size * (bbox[3]-bbox[1])
            image_b[(int(bbox[3] - crop_size_b)):int(bbox[3]), int(bbox[0]):(int(bbox[2])), :] = \
                image[(int(bbox[3] - crop_size_b)):int(bbox[3]), int(bbox[0]):(int(bbox[2])), :]
            #cv2.imwrite('crop_b.jpg', image_b)  # image:[255,255,3]
             
            crop_size_r = cfg.corners.crop_size * (bbox[2]-bbox[0])        
            image_r[int(bbox[1]):(int(bbox[3])), (int(bbox[2]-crop_size_r)):int(bbox[2]), :] = \
                image[int(bbox[1]):(int(bbox[3])), (int(bbox[2]-crop_size_r)):int(bbox[2]), :]
            #cv2.imwrite('crop_r.jpg', image_r)  # image:[255,255,3]
            #print('here')
            #time.sleep(3)

        if data == 'search':
            attentions = [np.zeros((1, cfg.atts.att_size, cfg.atts.att_size), dtype=np.float32)]  # 25 ???attention map??????
            # tl_heats_map
            ...
            #atts_map, x_int, y_int, x_float, y_float = self.create_attention_mask(attentions, cfg.TRAIN.ratios, bbox) # image:[255,255,3] x_int,y_int????????????????????????
            atts_map =[]

            xtl, ytl = bbox[0], bbox[1]  # ????????????255?????????            
            ...
            fxtl = (xtl * cfg.corners.Ratios)  # width_ratio:???55-->25???????????????            
            ...
            # visual bounding box
            #cv2.rectangle(image, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (0, 0, 255), thickness=1)  # ???[0,0,255]
            ...
            if valid:
                ...
                tl_tags[0] = max(0, min(ytl * cfg.corners.cor_size + xtl, cfg.corners.cor_size*cfg.corners.cor_size-1)) # ???????????? ytl????????????
                br_tags[0] = max(0, min(ybr * cfg.corners.cor_size + xbr, cfg.corners.cor_size*cfg.corners.cor_size-1))
            else:
                draw_gaussian(tl_valids[b_ind, category], [xtl, ytl], radius)  # ????????????masked_heatmap
        ...
        if x_int:
            ...
            tl_tags[0] = y_int * cfg.offs.off_size + x_int  # ???????????? ytl????????????
        else:
