...
class Augmentation:
    ...
    def __call__(self, image, bbox, size, data, gray=False):
        ...
        if data == 'template':
            ...
            image_l[0:(-1), int(0):(int(bbox[0]+crop_size_l)), :] =\
                image[0:(-1), int(0):(int(bbox[0]+crop_size_l)), :]
            ...
            image_t[int(0):(int(bbox[1]+crop_size_t)), 0:(-1), :] = \
                image[int(0):(int(bbox[1]+crop_size_t)), 0:(-1), :]
            #cv2.imwrite('crop_t.jpg', image_t)  # image:[255,255,3]
            
            crop_size_b = cfg.corners.crop_size * (bbox[3]-bbox[1])
            image_b[(int(bbox[3] - crop_size_b)):(-1), 0:(-1), :] = \
                image[(int(bbox[3] - crop_size_b)):(-1), 0:(-1), :]
            #cv2.imwrite('crop_b.jpg', image_b)  # image:[255,255,3]
             
            crop_size_r = cfg.corners.crop_size * (bbox[2]-bbox[0])        
            image_r[0:(-1), (int(bbox[2]-crop_size_r)):(-1), :] = \
                image[0:(-1), (int(bbox[2]-crop_size_r)):(-1), :]
