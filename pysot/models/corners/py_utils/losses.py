...
def _off_loss(off, gt_off, mask):
    num  = mask.float().sum()
    mask = mask.unsqueeze(2).expand_as(gt_off)# mask:[5,128,2]

    off    = off[mask] # off:[5,128,2]
    gt_off = gt_off[mask]
...
class CornerNet_Saccade_Loss(nn.Module):
    ...
    def forward(self, outs, targets):
        ...
        atts = [[_sigmoid(a) for a in att] for att in atts] # [5,1,16,16] [5,1,32,32] [5,1,64,64]*3
        atts = [[att[ind] for att in atts] for ind in range(len(gt_atts))] # [5,1,16,16]*3 [5,1,32,32]*3...
        ...
        off_loss = 0
        tl_offs  = [_tranpose_and_gather_feat(tl_off, gt_tl_ind) for tl_off in tl_offs] # [5,2,64,64]*3-->[5,128,2]
