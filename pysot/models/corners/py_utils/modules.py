...
class saccade_net(nn.Module):
    ...
    def _train(self, *xs):
        ...
        ups = [up[self.up_start:] for up in ups] # ups:[5,384,16,16] [5,384,32,32] [5,256,64,64] *3 乘3是因为不同层的特征

        tl_modules = [tl_mod_(cnv) for tl_mod_, cnv in zip(self.tl_modules, cnvs)]
        br_modules = [br_mod_(cnv) for br_mod_, cnv in zip(self.br_modules, cnvs)]
        tl_heats   = [tl_heat_(tl_mod) for tl_heat_, tl_mod in zip(self.tl_heats, tl_modules)] # [5,256,64,64]--> [5,80,64,64]*3
        br_heats   = [br_heat_(br_mod) for br_heat_, br_mod in zip(self.br_heats, br_modules)]
        tl_tags    = [tl_tag_(tl_mod)  for tl_tag_,  tl_mod in zip(self.tl_tags,  tl_modules)] # [5,1,64,64]*3
        br_tags    = [br_tag_(br_mod)  for br_tag_,  br_mod in zip(self.br_tags,  br_modules)]
        tl_offs    = [tl_off_(tl_mod)  for tl_off_,  tl_mod in zip(self.tl_offs,  tl_modules)] # [5,2,64,64]*3
        br_offs    = [br_off_(br_mod)  for br_off_,  br_mod in zip(self.br_offs,  br_modules)]
    ...
    def _test(self, *xs, no_att=False, **kwargs):
        image = xs[0]
        cnvs, ups = self.hg(image) # cnvs:[2,256,64,64],...
        ups = [up[self.up_start:] for up in ups]

        if not no_att:                                                               # ups[2,384,16,16] [2,384,32,32] [2,256,64,64] * 3
            atts = [att_mod_(up) for att_mod_, up in zip(self.att_modules[-1], ups[-1])] # atts:[2,1,16,16],[2,1,32,32],[2,1,64,64]
            atts = [torch.sigmoid(att) for att in atts]

        tl_mod = self.tl_modules[-1](cnvs[-1]) # cnvs:[2，256，64，64]*3
        br_mod = self.br_modules[-1](cnvs[-1])

        tl_heat, br_heat = self.tl_heats[-1](tl_mod), self.br_heats[-1](br_mod) # tl_mod:[2,256,64,64]  tl_heat:[2,80,64,64]
        tl_tag,  br_tag  = self.tl_tags[-1](tl_mod),  self.br_tags[-1](br_mod)
        tl_off,  br_off  = self.tl_offs[-1](tl_mod),  self.br_offs[-1](br_mod)
