...
class SiameseTracker(BaseTracker):
    def get_subwindow(self, im, pos, model_sz, original_sz, avg_chans, bbox):
        ...
        if bbox is not None:
            ...
            im_tm_t[int(0):(int(bbox[1]+crop_size_t)), 0:(-1), :] =\
                im_patch[int(0):(int(bbox[1]+crop_size_t)), 0:(-1), :]
            ...
            im_tm_l[0:(-1), int(0):(int(bbox[0]+crop_size_l)), :] = \
                im_patch[0:(-1), int(0):(int(bbox[0]+crop_size_l)), :]
            ...
            im_tm_b[(int(bbox[3] - crop_size_b)):(-1), 0:(-1), :] = \
                im_patch[(int(bbox[3] - crop_size_b)):(-1), 0:(-1), :]
            ...
            im_tm_r[0:(-1), (int(bbox[2]-crop_size_r)):(-1), :] = \
                im_patch[0:(-1), (int(bbox[2]-crop_size_r)):(-1), :]
